...
function* _deploy(callback) {
    ...
    _gen = _launchFormation(_bucket.name, _bucket.uri, (error) => {
        if (error != undefined) {
...
/**
 * Called on completion.
 * @param {string} name Name for the stack instance.
 * @param {string} bucket Bucket name where CloudFront template is stored.
 * @param {cbk} callback Error first callback.
 */
function* _launchFormation(name, bucket, callback) {
    log(`> Launching CloudFormation script... `, 'info');
    let _params = require('./cfpTemplate');
    _params.StackName = name;
    _params.TemplateURL = `${bucket}formation/lesslms.template`;
    ...
    cf.createStack(_params, (error, data) => {
        if (error) {
            ...
            _wait_g.s.start();
            _launchFormationPooler(data.StackId, (error, status) => {
                if (error) {
                    _gen.return(false); //OSLL: Error!
                    callback(error);
                } else {
                    log(`> CloudFormation process finished with status: "${status}" `, 'info');
    ...
    _timer_g = setTimeout(() => {
        ...
        cf.describeStacks(_params, (error, data) => {
            if (error) {
                ...
                let _stat = data.Stacks[0].StackStatus;
                _wait_g.s.message(`[${_wait_g.t++}s] CloudFormation status is: ${_stat}`);
                debugger;
                switch (_stat) {
