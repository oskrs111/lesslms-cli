 ...
 * @license MIT
 */
const _sourcesUri = 'https://api.github.com/repos/oskrs111/lesslms/releases/latest';
const fs = require('fs');
...
function* _deploy(callback) {
    ...
    let _args = {
        ...
        parameters: [{
            ...
            {
                ParameterKey: 'launchRegion',
                ParameterValue: configGet('region'),
                UsePreviousValue: false
            },
            {
        ...
        if (error) {
            ...
            _launchFormationPooler(data.StackId, (error, result) => {
                if (error) {
                    ...
                    //OSLL: Save CloudFormation results in serverles.json
                    for (let o of result.output) {
                        configSet(o.OutputKey, o.OutputValue);
                    }
